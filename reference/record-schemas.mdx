...
The public schema layer is built to stay stable enough for partner mapping and agent inspection while the internal implementation evolves freely underneath it. Field names are chosen to describe state, shape, and authority. Fields that would carry reviewer reasoning, scoring, or gate output are excluded at design time, per the [Disclosure boundary](/start/disclosure-boundary).
