...
<Info>
  There is no `terminated` or `withdrawn` lifecycle state today. How each outcome fits the state machine is in [Exception paths](/workflow/exception-paths). The names here are the contract. The criteria that move an engagement between them are not, per the [Disclosure boundary](/start/disclosure-boundary).
</Info>