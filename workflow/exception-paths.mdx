...
Every row carries one of two statuses:
...
`archived` is the only terminal lifecycle state. The outcome statuses that stop progress without a lifecycle transition are `declined`, `expired`, `voided`, `refunded`, `failed`, and `cancelled`.
