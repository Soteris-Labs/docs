...
Integrations should reason about engagements in the state vocabulary below. The names are stable enough to map partner-side status onto, and none of them encodes internal criteria. Where a state represents internal review, it is deliberately opaque: you can see that an engagement is in review, not why.
...
| `document_gate_open` | Required inputs are sufficient to proceed. | Inputs accepted; preparation can follow. |
...
<Note>
`cpa_gate` and `soteris_review` tell you an engagement is under review authority and that no released outcome exists yet. They do not tell you what is being evaluated or against what bar. Treat them as wait signals, not as inputs to your own logic.
</Note>
